...
router.get('/music', (req: express.Request, res: express.Response) => {
    MusicTable.getAllMusic()
        .then(data => {
            res.json({
                success: true,
                music: data
        ...
        .catch(err => {
            ErrorTable.postErrorLog(null, '[music] MusicTable.getAllMusic()', err)
            res.status(500).json({
                success: false,
                msg: 'Could not get from database'
            })
...
router.post('/getNotesData', (req: express.Request, res: express.Response) => {
    if (typeof req.body.id !== 'undefined' && req.body.id.length) {
        MusicTable.getNotesData(req.body.id + 1)
            .then(data => {
                res.json({
                    success: true,
                    data: data
            ...
            .catch(err => {
                ErrorTable.postErrorLog(null, '[music] MusicTable.getNotesData()', err)
                res.status(500).json({
                    success: false,
                    msg: 'Could not get notes data'
                })
            })
    } else {
        ErrorTable.postErrorLog(null, '[music] id isset check', 'id=' + req.body.id)
        res.status(500).json({
            success: false,
            msg: 'Could not get notes data'
        })
